...
const TransactionComponent = ({ tx }: { tx: Transaction }) => {
    const formatedDate = new Date(tx.createdAt).toLocaleDateString("fr-FR", {
        day: "numeric",
        month: "short",
        year: "numeric"
    })
    return (
        <div className='p-4 border-2 border-base-200 rounded-3xl flex items-center' >
            <div>
                {
                    tx.imageUrl && (
                        <ProductImage
                        ...
                        <div>
                            {
                                tx.type === "IN" ? (
                                    ...
                                    <div>
                                        <span className='text-error font-bold capitalize'>
                                            -{tx.quantity} {tx.unit}
                                        </span>
                                    </div>
                                )
